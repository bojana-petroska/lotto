...
class LottoTest {
    ...
    fun `if lotto ticket have over 6 numbers should be thrown an error`() {
        assertThrows<IllegalArgumentException> {
            Lotto(LottoNumbers.of(listOf(1, 2, 3, 4, 5, 6, 7)))
        }
    }
    ...
    fun `if lotto ticket should have 6 numbers`() {
        ...
        assertTrue(lotto.numbers.numbers.size == NUMBERS_SIZE)
    }

    @Test
    fun `if lotto ticket numbers are not unique thrown`() {
        assertThrows<IllegalArgumentException> {
            Lotto(LottoNumbers.of(listOf(1, 2, 3, 4, 5, 5)))
        }
    }
    ...
    fun `if lotto ticket each number is not in range between 1 and 45 thrown`() {
        assertThrows<IllegalArgumentException> {
            Lotto(LottoNumbers.of(listOf(1, 2, 3, 4, 5, 46)))
        }
    }

    @Test
    fun `lotto ticket should return match count`() {
        val lotto = Lotto(LottoNumbers.of(listOf(5, 6, 7, 8, 9, 10)))
        val winningNums = listOf(1, 2, 3, 4, 5, 6)
        val matchCount = lotto.matchCount(LottoNumbers.of(winningNums))
    ...
    fun `lotto ticket should check bonus number`() {
        val lotto = Lotto(LottoNumbers.of(listOf(5, 6, 7, 8, 9, 10)))
        val bonusNum = 11
        val result = lotto.containsBonusNum(BonusNumber.of(bonusNum, lotto.numbers.numbers))
        assertTrue(!result)
    }
}
